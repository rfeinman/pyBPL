...
# has to do evertying concept did 
class Type(object):
...
class TypeDist(AbstractTypeDist):
    """
    A TypeDist is a probabilistic program that samples character types
    from the prior. It can also compute the log-likelihood of character types.
    'TypeDist' defines the prior distribution P(Type)
    ...
    def score_relation_type(self, r, prev_parts):
        ...
        if r.category == 'unihist':
            ...
            # TODO : why do I have to make a tensor here
            ll = ll + dist.Categorical(probs=probs).log_prob(torch.tensor(r.attach_ix))
            ...
            if r.category == 'mid':
                ...
                # TODO SAME HERE
                ll = ll + dist.Categorical(probs=probs).log_prob(torch.tensor(r.attach_subix))
                # score the type-level spline coordinate
                _, lb, ub = bspline_gen_s(self.lib.ncpt, 1)
                # TODO SAME HERE
                ll = ll + dist.Uniform(lb, ub).log_prob(torch.tensor(r.eval_spot))
        else:
            raise TypeError('invalid relation')
    ...
    def sample_type(self, k=None):
        ...
        """
        # c is now a Type instead of a Concept
        c = super(TypeDist, self).sample_type(k)


        # Should be replaced with Token Dist
...
class Token_ImDist(ConceptToken):
    ...
    def __init__(self, P, R, affine, epsilon, blur_sigma, parameters):
        super(Token_ImDist, self).__init__(P, R)
        for ptoken in P:
...
class TokenDist(AbstractTokenDist):
    ...
    def __init__(self, k, P, R, lib):
        super(TokenDist, self).__init__(k, P, R)
        for p in P:
            assert isinstance(p, Stroke)
        assert isinstance(lib, Library)
        self.lib = lib
    ...
    def sample_token(self):
        ...
        concept_token = super(TokenDist, self).sample_token()
