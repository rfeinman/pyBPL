...
class CPD(object):
	...
	def sample_number(libclass):
		pkappa = Variable(torch.squeeze(torch.Tensor(lib['pkappa'])))
	...
	def sample_relation_type(libclass,prev_strokes):
		...
		if nprev == 0:
			...
			mixprob = Variable(torch.squeeze(torch.Tensor(libclass['rel']['mixprob'])))
	...
	def sample_sequence(libclass,ns,nsub=[]): #gives template.S[i].ids
		...
		if nsub = []:
			nsub = CBD.sample_nsub(libclass,ns)

		sq = Variable(torch.zeros(nsub))
		pStart = torch.exp(torch.squeeze(torch.Tensor(libclass['logStart'])))
		sq[0] = pyro.sample('start_sid', dist.categorical, Variable(pStart), one_hot=False) + 1
		for bid in range(1,nsub):
			prev = sq[bid-1]
			pT = libclass.pT(prev) #need to implement #TODO
			sq[bid] = pyro.sample('sids',dist.categorical, Variable(pT), one_hot=False) + 1 

		return sq
		#seq = sq.data
		#return seq #seq is a numpy array so i can interate over it
	...
	def sample_shape_type(libclass,subid):
		...
		ncpt = 5
		bspline_stack = torch.zeros(5,2,k)

		for i in subid.data:
			mu = torch.squeeze(torch.Tensor(libclass['shape']['mu']))
			Cov = torch.squeeze(torch.Tensor(libclass['shape']['Sigma']))[:,:,i]
			...
			bspline_stack[:,:,i] = rows.view([ncpt,2])
	...
	def sample_invscales_type(libclass,subid):
		...
		theta = torch.squeeze(torch.Tensor(libclass['scale']['theta']))[subid.data - 1] #because indexing

		#pyro gamma dist uses rate parameter, which is inverse of scale parameter
		invscales = pyro.sample('invscales',dist.gamma,Variable(theta[:,0]), Variable(1/theta[:,1]))
		return invscales
	...
	def sample_relation_token(libclass, eval_spot_type):
		...
		sigma_attach = torch.squeeze(torch.Tensor(libclass['tokenvar']['sigma_attach']))
	...
	def sample_position(libclass, R, prev_strokes):  #check that this does what I want, slicewise
		...
		sigma_x = Variable(torch.squeeze(torch.Tensor(libclass['rel']['sigma_x'])))
		sigma_y = Variable(torch.squeeze(torch.Tensor(libclass['rel']['sigma_y'])))
	...
	def sample_shape_token(libclass, bspline_stack):
		...
		sigma_shape = torch.squeeze(torch.Tensor(libclass['tokenvar']['sigma_shape']))
		outstack = bspline_stack + Variable(sigma_shape) * pyro.sample('shape_var',dist.normal,Variable(torch.zeros(sz)),Variable(torch.ones(sz)))
		return outstack

	def sample_invscale_token(libclass, invscales_type):
		sz = invscales_type.size
		sigma_invscale = torch.squeeze(torch.Tensor(libclass['tokenvar']['sigma_invscale']))
	...
	def sample_affine(libclass):
		...
		S_scale = Variable(torch.squeeze(torch.Tensor(libclass['affine']['Sigma_scale'])))
		...
		s_x = Variable(torch.squeeze(torch.Tensor(libclass['affine']['sigma_xtranslate'])))
		s_y = Variable(torch.squeeze(torch.Tensor(libclass['affine']['sigma_ytranslate'])))
