...
def sample_sequence(libclass,ns,nsub=[]): #gives template.S[i].ids
	...
	sq = [pyro.sample('start_sid', dist.categorical, Variable(pStart), one_hot=False) + 1]
...
def sample_invscales_type(libclass,subid):
	...
	theta = torch.squeeze(torch.Tensor(libclass['scale']['theta'][0,0]))[subid.data - 1] #because indexing
...
def sample_shape_token(libclass, bspline_stack):
	...
	sigma_shape = torch.squeeze(torch.Tensor(libclass['tokenvar']['sigma_shape'][0,0]))
...
def sample_affine(libclass):
	...
	s_x = Variable(torch.squeeze(torch.Tensor(libclass['affine']['sigma_xtranslate'][0,0])))
	s_y = Variable(torch.squeeze(torch.Tensor(libclass['affine']['sigma_ytranslate'][0,0])))
